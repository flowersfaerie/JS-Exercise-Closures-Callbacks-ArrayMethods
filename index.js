...
function processContains(item, list, callback) {
	const itemFilter = list.includes(item);
	return callback(itemFilter);
...
function processDuplicateFree(list, callback) {
	let dupeFree = list.filter((item, index) => {
		(item, index, list.indexOf(item), list.indexOf(item) === index);
...
function firstNamesAllCaps(runners) {
	const capFirstNames = runners.map((runners) => {return `${runners.first_name}`.toUpperCase()});
...
function getRunnersByTShirtSize(runners, tShirtSize) {
	let runnersBySize = runners.filter((item, index) => {
...
function tallyUpDonations(runners) {
	let tally = runners.reduce((acc, cur) => acc + cur.donation, 0);
	return tally;
}
